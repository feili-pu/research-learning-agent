...
const modeDescriptions: Record<Mode, string> = {
  discovery: "先用大模型理解研究意图并生成学术检索式，再从 Semantic Scholar、Crossref、arXiv 和 OpenAlex 搜索候选论文。",
...
const modePromptInstructions: Record<Mode, string> = {
  discovery: "外部文献发现会先调用 LLM，把研究方向和关注重点整理成多条论文库友好的英文学术检索式，再搜索外部源，并要求候选论文标题、摘要、关键词或研究领域覆盖检索意图。",
    ...
    <section className="result-stack">
      <article className="paper-panel">
        ...
        {result.queries_used.length > 0 && (
          <div className="query-plan">
            <strong>
              实际检索式 · {result.query_planner === "llm" ? "LLM 智能规划" : result.query_planner}
              {result.planner_model ? ` · ${result.planner_model}` : ""}
            </strong>
            <div>
